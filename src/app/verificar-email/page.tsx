...
export default function VerificarEmailPage() {
  ...
  useEffect(() => {
    ...
    supabase.from("verification_tokens").select("*").eq("token", token).single().then(async ({ data, error }) => {
      ...
      if (data.verified) {
        setStatus("success")
        setMessage("Tu email ya fue verificado anteriormente.")
        return
      }
      // Mark as verified
      await supabase.from("verification_tokens").update({ verified: true }).eq("token", token)
      setStatus("success")
      setMessage("¡Email verificado! Ya podés usar La Percha.")
    })
  }, [token])
  ...
  return (
    <main className="min-h-screen bg-bg-page flex items-center justify-center px-6">
      <div className="w-full max-w-sm text-center space-y-6">
        ...
        <Link href="/home" className="inline-flex items-center gap-2 px-6 py-3 bg-brand text-white font-semibold rounded-full text-sm hover:bg-brand-hover transition-colors">
          Ir a la tienda
        </Link>
      </div>
    </main>
