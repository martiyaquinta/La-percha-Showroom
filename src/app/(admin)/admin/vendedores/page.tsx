...
import { Check, X, User, Copy, CheckCheck } from "lucide-react"

export default function VendedoresPage() {
  const { vendors, approveVendor, rejectVendor } = useAdminStore()
  ...
  return (
    <div className="p-5 lg:p-7 pt-20 lg:pt-7 space-y-5 max-w-4xl">
      <div>
        ...
        <p className="text-sm text-text-muted mt-1">Gestioná las solicitudes de vendedores</p>
      </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <p className="text-sm font-semibold text-text-strong">{v.name}</p>
                    {v.status === "pending" ? (
                  ...
                  <div className="flex flex-wrap items-center gap-3 mt-2">
                    <span className="text-xs text-text-muted">{v.products} productos</span>
                    <span className="text-xs text-text-muted">{v.createdAt}</span>
                    <button onClick={() => copyCBU(v.cbu, v.id)}
                      className="inline-flex items-center gap-1 text-[11px] text-matcha-600 font-medium hover:text-matcha-700 transition-colors">
